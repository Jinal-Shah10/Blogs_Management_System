...
$v=$_SESSION["User"];
$querylw = 'select followed_user_id from follow where following_user_id = '.$v.' order by date desc';
...
echo '<div style="height:500px; width:50%; font-size:25px; font_weight:800; text-align:center; background:#343641; color:white; overflow: scroll; margin: 0 25% 0 25%"><p style="background-color:#1eb7f0; font-size:30px;">following</p>';
while ($rowlw = $resultlw->fetch_assoc())
{
	$queryu='select * from user where account_id ='.$rowlw["followed_user_id"];
